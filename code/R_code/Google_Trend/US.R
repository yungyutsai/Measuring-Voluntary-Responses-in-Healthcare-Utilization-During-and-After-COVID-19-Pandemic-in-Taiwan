...
for(x in geo){
  for(y in key){
    trends <- gtrends(y, geo = x, time = whole, tz = "Etc/GMT-6")$interest_over_time
    data <- trends[,1:4]
    if(x == geo[1] & y == key[1]){
      monthly.data <- data
    }
...
save.name <- paste0(rawdata,"/google_trend_monthly_US.dta")
...
for(x in geo){
  for(y in key){
    for(z in time){
      tryCatch({ 
        trends <- gtrends(y, geo = x, time = z, tz = "Etc/GMT-6")$interest_over_time
        data <- trends[,1:4]
...
save.name <- paste0(rawdata,"/google_trend_daily_US.dta")
