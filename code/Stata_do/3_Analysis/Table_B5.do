...
if "`y'" == "opd" & `i' == 1{
outreg2 using "$table/temp/Table_B5_`x'", ///
...
clear
import delimited "$table/temp/Table_B5_`x'.txt"
save "$table/temp/Table_B5_`x'.dta", replace

}

use "$table/temp/Table_B5_total.dta", clear
ap using "$table/temp/Table_B5_flu.dta"
ap using "$table/temp/Table_B5_non_flu.dta"
...
export excel using "$table/Table_B5.xlsx", replace

drop in 13

texsaveyt 	_all using "$tex/tables/TabB5_temp.tex", replace ///
...
filefilter "$tex/tables/TabB5_temp.tex" "$tex/tables/TabB5.tex", from("&&&&&&&&") to("") replace
cap rm "$tex/tables/TabB5_temp.tex"
