...
if "`y'" == "opd" & `i' == 1{
outreg2 using "$table/temp/Table_B4a_`x'", ///
...
if "`y'" == "opd" & `i' == 1{
outreg2 using "$table/temp/Table_B4b_`x'", ///
replace title("Outcomes: `x'") ctitle(`y') nocon keep(TxPandemic TxCOVIDFree) dec(2) alpha(0.001, 0.01, 0.05) eform st(coef se ci_low ci_high)
}
else{
outreg2 using "$table/temp/Table_B4b_`x'", ///
...
import delimited "$table/temp/Table_B4a_`x'.txt", clear
save "$table/temp/Table_B4a_`x'.dta", replace
import delimited "$table/temp/Table_B4b_`x'.txt", clear
save "$table/temp/Table_B4b_`x'.dta", replace
}

clear
ap using "$table/temp/Table_B4a_total.dta"
ap using "$table/temp/Table_B4b_total.dta"
ap using "$table/temp/Table_B4a_flu.dta"
ap using "$table/temp/Table_B4b_flu.dta"
ap using "$table/temp/Table_B4a_non_flu.dta"
ap using "$table/temp/Table_B4b_non_flu.dta"


keep if inrange(_n,4,12) | inrange(_n,23,24) | inrange(_n,27,28) | ///
		inrange(_n,36,44) | inrange(_n,55,56) | inrange(_n,59,60) | ///
		inrange(_n,68,76) | inrange(_n,87,88) | inrange(_n,91,92) | _n == 94

foreach var of varlist v2-v9{
	replace `var' = subinstr(`var',"*","",.)
	replace `var' = subinstr(`var',"(","",.)
	replace `var' = subinstr(`var',")","",.)
	replace `var' = "[" + `var'[_n-1] + "," + `var' + "]" if mod(_n,13) == 5 | mod(_n,13) == 9
	replace `var' = "{" + `var'[_n-1] + "," + `var' + "}" if mod(_n,13) == 11 | mod(_n,13) == 0
}

drop if mod(_n,13) == 3 | mod(_n,13) == 4 | mod(_n,13) == 7 | mod(_n,13) == 8 | mod(_n,13) == 10 | mod(_n,13) == 12

gen row = _n
replace row = row - 2.5 if mod(row,7) == 6
sort row
drop row

replace v1 = "\multicolumn{9}{@{}l@{}}{\textbf{Panel A:} All diseases}" in 1 
replace v1 = "\multicolumn{9}{@{}l@{}}{\textbf{Panel B:} ILI diseases}" in 8
replace v1 = "\multicolumn{9}{@{}l@{}}{\textbf{Panel C:} Non-ILI diseases}" in 15
replace v1 = "\$Y_{2020} \times Pandemic$" if v1 == "TxPandemic"
replace v1 = "\$Y_{2020} \times CovidFree$" if v1 == "TxCOVIDFree"
replace v1 = "Cluster at county level" if mod(_n,7) == 3 | mod(_n,7) == 6
replace v1 = "Cluster at year-week level" if mod(_n,7) == 4 | mod(_n,7) == 0

export excel using "$table/Table_B4.xlsx", replace


texsaveyt 	_all using "$tex/tables/TabB4_temp.tex", replace ///
			title ("Robustness check: Clustering levels of standard errors (multi-period DID design)") nonames ///
			...
			hlines(0 7 14 22 22) nofix size(footnotesize) align(@{}lcccccccc@{}) ///
			label(did_table2_app) frag rh(1.25) cs(1) ///
...
filefilter "$tex/tables/TabB4_temp.tex" "$tex/tables/TabB4.tex", from("&&&&&&&&") to("") replace
cap rm "$tex/tables/TabB4_temp.tex"
